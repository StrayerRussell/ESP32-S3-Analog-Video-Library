...
bool CVBS::init(const PinConfig pins, CVBSMode mode, int bits) {
  this->pins = pins;
  this->mode = mode;
  this->bits = bits;
  backBuffer = 0;
...
// signals such as CVBS video which require consistent data flow and does not require external sync
  ...
  if (bits == 8) {
    int pins[8] = {
      this->pins.pins[0], this->pins.pins[1], this->pins.pins[2],
      this->pins.pins[3], this->pins.pins[4], this->pins.pins[5],
      this->pins.pins[6], this->pins.pins[7]
    };
    ...
    int pins[16] = {
      this->pins.pins[0], this->pins.pins[1], this->pins.pins[2], this->pins.pins[3],
      this->pins.pins[4], this->pins.pins[5], this->pins.pins[6], this->pins.pins[7],
      this->pins.pins[8], this->pins.pins[9], this->pins.pins[10], this->pins.pins[11],
      this->pins.pins[12], this->pins.pins[13], this->pins.pins[14], this->pins.pins[15]
    };
    for (int i = 0; i < bits; i++)
      if (pins[i] >= 0)
        attachPinToSignal(pins[i], LCD_DATA_OUT0_IDX + i);
...
bool CVBS::ispositioninvalid(uint32_t x, uint32_t y) {
  ...
  if (x < 0 || x > mode.VisibleLineSamples || y < 0 || y > mode.VisibleLines) return true; // position is invalid
